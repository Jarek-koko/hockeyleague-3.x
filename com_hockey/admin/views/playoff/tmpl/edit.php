...
{
    if (task == 'playoff.cancel') {
        Joomla.submitform(task, document.getElementById('playoff-form'));
    }
    else {

        if (task != 'playoff.cancel' && document.formvalidator.isValid(document.id('playoff-form'))) {
            Joomla.submitform(task, document.getElementById('playoff-form'));
        }
        else {
...
<form action="<?php echo JRoute::_('index.php?option=com_hockey&layout=edit&id=' . (int) $this->item->id); ?>" method="post" 
      name="adminForm" id="playoff-form" class="form-validate">
<div class="row-fluid">
...
<div class="span2">
    ...
    <div class="control-group">
     ...
     <?php echo $this->form->getInput('type_of_match',null , 1); ?>
