 ...
 * Hockey model.
 */
class HockeyModelScheduler extends JModelLegacy
{
    protected $id_sezon;
        ...
        if (!isset($this->_item)) {
            ...
            if ($this->_item === false) {
                ...
                $query = $db->getQuery(true)
                    ...
                    ->where('M.id_system = '. $db->Quote($this->id_sezon) .' AND M.type_of_match=0 AND M.state=1 ')
                    ->order('M.id_kolejka, M.data');
    ...
    public function getMatchdays()
    {
        ...
        if (!isset($this->_matchday)) {
            $cache = JFactory::getCache('com_hockey', '');

            $id = 'matchday-list-' . $this->id_sezon;

            $this->_matchday = $cache->get($id);

            if ($this->_matchday === false) {
          
                $db = $this->getDbo();
                $query = $db->getQuery(true)
                    ->select('id_kolejka ')
                    ->from('#__hockey_match')
                    ->where('id_system=' . $db->Quote($this->id_sezon). ' AND type_of_match=0 AND state=1')
                    ->group('id_kolejka');
                $db->setQuery($query);
